...
def contar():
    ...
    c = len(textoRodape.get('1.0', END))
    c = c - 1

    contTitulo.config(text=a)
    contDescricao.config(text=b)
    contRodape.config(text=c)
...
def salvar():
    ...
    tag = str(textoRodape.get(1.0, END))
...
contRodape = Label(master, text="", font=("Calibri", 14), background="#FFFFFF")
contRodape.place(width=97, height=66, x=836, y=664)
...
textoRodape = Text(frameTag, font=("Calibri", 16), selectbackground="#F5F799", selectforeground="black",
                 yscrollcommand=barraLateral2.set)
textoRodape.place(width=786, height=222, x=0, y=0)

# Configura a barra lateral
barraLateral1.config(command=textoDescricao.yview)
barraLateral2.config(command=textoRodape.yview)
